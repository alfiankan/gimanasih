...
import { DotProductConcept } from '../components/dot-product/DotProductConcept'
import { DotProductSimulator } from '../components/dot-product/DotProductSimulator'
import { DotProductGame } from '../components/dot-product/DotProductGame'
import { DotProductQuiz } from '../components/dot-product/DotProductQuiz'
import { Button } from '@/components/ui/button'
import { Card } from '@/components/ui/card'
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@/components/ui/tabs'
import { AuthGate } from '../components/AuthGate'

interface DotProductLessonProps {
  onBack: () => void
  onPass?: () => void
  currentXp: number
...
export const DotProductLesson: React.FC<DotProductLessonProps> = ({ onBack, onPass, currentXp, onGainXp, isLoggedIn }) => {
  ...
  return (
    <div className="lesson-container p-6 md:pl-[272px] md:pt-8 md:pr-8 max-w-5xl mx-auto w-full mb-24 md:mb-8 flex flex-col gap-5 animate-slide-up select-none">
      <Card glass className="flex items-center justify-between p-4 rounded-2xl border-white/5 bg-slate-950/60">
        <Button
          variant="outline"
          size="sm"
          onClick={onBack}
        ...
        <div className="text-center hidden sm:block">
          <span className="text-[9px] font-extrabold text-neon-secondary uppercase tracking-widest block mb-0.5">Math & Algorithms</span>
          <h1 className="text-base md:text-lg font-extrabold text-foreground tracking-tight leading-none">Dot Product</h1>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="bg-amber-500/10 text-amber-400 border border-amber-500/25 text-[10px] font-extrabold uppercase tracking-wider px-2.5 py-1 rounded-lg flex items-center gap-1.5">
            <Zap size={12} className="fill-amber-400/20" /><span>{currentXp} XP</span>
          </div>
          {quizPassed ? (
      ...
      <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val as TabType)} className="w-full flex flex-col gap-4">
        ...
        <main className="lesson-content-area min-h-[380px] w-full flex flex-col">
          <TabsContent value="concept" className="mt-0">
            <DotProductConcept onComplete={() => setActiveTab('simulator')} />
          </TabsContent>

          <TabsContent value="simulator" className="mt-0">
            <DotProductSimulator onComplete={() => setActiveTab('game')} />
          </TabsContent>
